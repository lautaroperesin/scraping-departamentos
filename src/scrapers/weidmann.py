...
class WeidmannScraper(BaseScraper):
    def scrapear(self):
        print(f"Buscando en Weidmann ({self.url_base})...")
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Error {response.status_code} al conectar con Weidmann")
                return []
            ...
            for card in cards:
                try:
                    ...
                    resultados_normalizados.append({
                        'id': f"WEIDMANN_{id_publicacion}",
                        'titulo': titulo,
                        'precio': precio,
                        'descripcion': descripcion,
                        'link': link,
                        'portal': 'Weidmann Inmobiliaria'
                    })

                except Exception as e:
                    print(f"Error parseando tarjeta Weidmann: {e}")
                    continue

        except Exception as e:
            print(f"Error scrapeando Weidmann: {e}")
