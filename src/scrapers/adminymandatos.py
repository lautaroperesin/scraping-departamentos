...
class AdminymandatosScraper(BaseScraper):
    def scrapear(self):
        print(f"Buscando en Admin. y Mandatos ({self.url_base})...")
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"Error {response.status_code} al conectar con Admin. y Mandatos")
                return []
            ...
            for card in cards:
                try:
                    ...
                    resultados_normalizados.append({
                        'id': f"ADMINYMANDATOS_{id_publicacion}",
                        'titulo': titulo,
                        'precio': precio,
                        'descripcion': descripcion,
                        'link': link,
                        'portal': 'Administraciones y Mandatos'
                    })

                except Exception as e:
                    print(f"Error parseando tarjeta Admin. y Mandatos: {e}")
                    continue

        except Exception as e:
            print(f"Error scrapeando Admin. y Mandatos: {e}")
